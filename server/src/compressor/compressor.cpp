...
std::vector<uint8_t> LibArchiveCompressor::compress(const std::vector<FileEntry>& files) {
    ...
    try {
        ...
        std::string temp_filename = "/tmp/libarchive_temp_" + std::to_string(time(nullptr)) + ".archive";
        
        if (archive_write_open_filename(a, temp_filename.c_str()) != ARCHIVE_OK) {
            throw std::runtime_error("Failed to open archive for writing: " + std::string(archive_error_string(a)));
        }
        ...
        for (const auto& file : files) {
            ...
            if (file_data.empty() && !file.source_path.empty()) {
                file_data = loadFileFromDisk(file.source_path);
            }
            ...
            if (archive_write_header(a, entry) != ARCHIVE_OK) {
                ...
                throw std::runtime_error("Failed to write file header: " + std::string(archive_error_string(a)));
            }

            if (!file_data.empty()) {
                la_ssize_t bytes_written = archive_write_data(a, file_data.data(), file_data.size());
                if (bytes_written < 0) {
                    archive_entry_free(entry);
                    throw std::runtime_error("Failed to write file data: " + std::string(archive_error_string(a)));
                }
            }
...
std::vector<FileEntry> LibArchiveCompressor::extract(const std::vector<uint8_t>& archive_data) {
    std::string temp_filename = "/tmp/libarchive_extract_" + std::to_string(time(nullptr)) + ".archive";
    std::ofstream temp_file(temp_filename, std::ios::binary);
    if (!temp_file) {
        throw std::runtime_error("Cannot create temporary file for extraction");
    }
    temp_file.write(reinterpret_cast<const char*>(archive_data.data()), archive_data.size());
    temp_file.close();
    ...
    try {
        ...
        if (archive_read_open_filename(a, temp_filename.c_str(), 65536) != ARCHIVE_OK) {
            throw std::runtime_error("Failed to open archive for reading: " + std::string(archive_error_string(a)));
        }
        ...
        while (archive_read_next_header(a, &entry) == ARCHIVE_OK) {
            ...
            if (file_size > 0) {
                ...
                la_ssize_t bytes_read = archive_read_data(a, file.data.data(), file_size);
                if (bytes_read < 0) {
                    throw std::runtime_error("Failed to read file data: " + std::string(archive_error_string(a)));
                }
...
std::vector<uint8_t> LibArchiveCompressor::loadFileFromDisk(const std::string& path) const {
